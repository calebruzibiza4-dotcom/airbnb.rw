...
function ProfileMenu({ open, onToggle, onLogIn, onSignUp }: { open: boolean; onToggle: () => void; onLogIn: () => void; onSignUp: () => void }) {
  return (
    <div className="relative shrink-0">
      ...
      {open ? (
        <div role="menu" className="absolute right-0 top-full z-20 mt-2 w-52 origin-top-right animate-[dropdown_200ms_ease-out] rounded-2xl border border-slate-200 bg-white p-2 shadow-xl shadow-slate-900/10">
          ...
          <a href="#help" role="menuitem" className="block rounded-xl px-3 py-2.5 text-sm text-slate-700 transition duration-200 hover:bg-slate-100 focus-visible:bg-slate-100 focus-visible:outline-none">Help Center</a>
        </div>
      ) : null}
    </div>
  );
